# 📘 Map Servers — Combined README (Historical Timeline + Geolocation)
...
Both servers expose tool functions with clear JSON schemas (function-level, no `self`), MelRok‑style docstrings, and brief inline comments. Each server ships with a minimal **Agent** and optional **Gradio UI** for interactive testing.
...
- Tools have MelRok‑style docstrings + brief inline comments.  
...
- POI `type` values must match exactly; feel free to extend `POIS`.  
